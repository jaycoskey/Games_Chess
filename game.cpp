...
void Game::printConciseMatchSummary(vector<GameState> &gss) {
    ...
    for (auto [gs, count] : gsHistogram) {
        cout << "\t" << std::setw(5) << std::left << std::setfill(' ') << count
             << "instances: " << gs;
    }
}
...
const GameState Game::gameLoop() {
    ...
    while (true) {
        ...
        ExtMove extMove = Move::getPlayerMove(Player::playerType(c), _board, c,
                                              validPlayerMoves);
        ...
        if (extMove.optMove == std::nullopt) {
            if (extMove.isDrawClaim) {
                ...
                result = GameState{GameEnd::Draw, WinType::None, drawFlags};
                break;
            } else {
                ...
                if ((agreedGameEnd == GameEnd::WinBlack && c == Color::White) ||
                    ...
                    assert((agreedGameEnd == GameEnd::WinBlack &&
                            c == Color::Black) ||
                           (agreedGameEnd == GameEnd::WinWhite &&
                            c == Color::White));
                    winType = WinType::Agreement;
                }
                result =
                    GameState{agreedGameEnd, winType,
                              agreedGameEnd == GameEnd::Draw ? Draw_Agreement
                                                             : Draw_None};
                break;
            }
        } else {
            ...
            cout << std::setw(4) << std::left << std::setfill(' ') << oss.str()
                 << " Moved: " << move << "\n";
            cout << "-------------------------\n";
            move.apply(_board);
            ...
            result = GameState{_board, c, extMove.isDrawClaim,
                               _validPlayerMovesCache};
...
void Game::play(Short autoReplayCount, PlayerType wPlayer, PlayerType bPlayer) {
    ...
    for (Short gameNum = 1; autoReplayCount == 0 || gameNum <= autoReplayCount;
         ++gameNum)
    {
        cout << "=========================\n";
        ostringstream oss;
