...
public:
    ...
    static const string           history_to_pgn();
...
private:
    ...
    static ExtMove _parseMoveInAlgNotation(const Board& b, Color c, const string& input) noexcept(false);
...
PieceType2IsAttackingRule Move::_createIsAttackingRules()
{
    ...
    pt2cr[PieceType::King] =
        [&](const Board& b, const Piece& attacker, const Pos& tgtPos)
            { return isAttacking(b, attacker, tgtPos, allDirs,    1); };
    pt2cr[PieceType::Queen] =
        [&](const Board& b, const Piece& attacker, const Pos& tgtPos)
            { return isAttacking(b, attacker, tgtPos, allDirs,    0); };
    pt2cr[PieceType::Rook] =
        [&](const Board& b, const Piece& attacker, const Pos& tgtPos)
            { return isAttacking(b, attacker, tgtPos, orthoDirs,  0); };
    pt2cr[PieceType::Bishop] =
        [&](const Board& b, const Piece& attacker, const Pos& tgtPos)
            { return isAttacking(b, attacker, tgtPos, diagDirs,   0); };
    pt2cr[PieceType::Knight] =
        [&](const Board& b, const Piece& attacker, const Pos& tgtPos)
            { return isAttacking(b, attacker, tgtPos, knightDirs, 1); };
